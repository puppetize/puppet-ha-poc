...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  %w{puppet1 puppet2}.each do |name|
    config.vm.define name do |puppet|
      ...
      puppet.vm.provision "shell", inline: "wget http://apt.puppetlabs.com/puppetlabs-release-wheezy.deb && dpkg -i puppetlabs-release-wheezy.deb && apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -qq puppet"
